...
# BUG FIX: this is set once, when the dashboard process starts, and used to scope every
# KPI/chart/feed row to "since this dashboard has been up" — matching what the UI actually
# labels them ("Total Alerts (session)"). Previously nothing scoped by session at all, so
# a fresh demo run displayed the ENTIRE lifetime of alerts.jsonl — every test run across
# every day it had ever been used — inflating "Total Alerts" and "Critical Alerts" into
# the tens of thousands and making a live attack look like background noise.
SESSION_START = time.time()

MAX_FEED_ROWS = 50
ACTIVE_WINDOW_SECONDS = 60          # "Active Alerts" = alerts in the last minute
# BUG FIX: was 5 seconds, exactly equal to the controller's heartbeat interval (POLL_INTERVAL
# in controller.py) — any small scheduling jitter would make the dot flicker to IDLE even
# while the enclave was genuinely running. Widened to comfortably cover a couple of missed
# ticks. This only matters at all now that something actually writes activity.jsonl — see
# PassiveThreatController._ping_activity() / _monitor() in controller.py. Before that fix,
# no ENCLAVE_ALIVE_WINDOW_SECONDS value could have made this correct, because the file the
# dashboard reads here was never being written by anything.
ENCLAVE_ALIVE_WINDOW_SECONDS = 15
...
def build_dashboard_data():
    ...
    # BUG FIX: scope every KPI/chart/feed row to the current dashboard session (see
    # SESSION_START above) instead of the entire history of the file.
    ...
    <div class="flex gap-6">
        ...
        <div class="w-96 flex flex-col gap-6">
            ...
            <div class="bg-[#131A32] p-5 rounded-lg border border-slate-800 text-xs space-y-2">
                ...
                <div class="flex justify-between">
                    <span class="text-slate-400">Detection</span>
                    <span class="font-semibold text-slate-500">Adaptive gate + Random Forest + rule-based detectors (scan, DGA, tunnel, beacon, slowloris)</span>
                </div>
            </div>
        </div>
    </div>
